...
{
    public class CateringCache
    {
        public CateringCache()
        {
            IsCateringMenuCacheSetup = false;
        }

        //Properties
        private bool IsCateringMenuCacheSetup;
        public string[] Categories { get; private set; } = new string[] { "Breakfast", "Lunch", "Salads", "Large Group Salads", "Soups", "Desserts", "Drinks" };

        private CateringMenu _Menu { get; set; }
        public CateringMenu Menu
        {
            get
            {
                if (!IsCateringMenuCacheSetup)
                    Setup();
        ...
        //Methods
        //  Setup will be called in Startup.cs by the DbInitializer.Initialize
        //  IMPORTANT: Do NOT call this in a constructor unless other references are removed. Otherwise, it will run multiple times.
        public void Setup()
        {
            Menu = new CateringMenu();
            IsCateringMenuCacheSetup = true;
